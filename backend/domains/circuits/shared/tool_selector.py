...
class SubDomainCapability:
    ...
    def matches(self, question_lower: str) -> int:
        return sum(1 for kw in self.keywords if kw in question_lower)
...
register_capability(SubDomainCapability("digital_logic", ["yosys", "icarus", "verilator"], "Digital logic: truth tables, boolean algebra, HDL synthesis.", ["truth table", "boolean", "logic gate", "nand", "nor", "xor", "k-map", "flip flop", "flip-flop", "mux", "decoder", "adder", "counter", "binary", "vcd", "waveform", "jk", "dff", "register", "clock", "sequential", "verilog", "vhdl"]))
...
def _keyword_fallback_select(question_lower: str) -> List[ToolSelection]:
    ...
    if not scored:
        return [ToolSelection("analog_sim", "ngspice",
            "No keyword match — defaulting to analog SPICE pipeline")]
