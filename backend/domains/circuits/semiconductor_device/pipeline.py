"""
Semiconductor Device Pipeline
==============================
Analyzes semiconductor device physics questions using LLM-driven analysis.
...
_LLM_PROMPT = """You are a semiconductor device physics expert. Given a user question, produce a JSON object with analysis results.

Return JSON with these fields:
- "system_type": short name (e.g. "NMOS Transistor", "PN Junction Diode")
...
def _build_model_parameters(plan):
    """
    Convert a semiconductor-device plan's metrics into structured parameters
    for the Seemulator Formulated Model pane. No deterministic solver backs
    this domain (LLM-only); edits are applied directly without recomputation.
    """
    return [
        {
...
def _to_standardized(result):
    return {
        "sub_domain": "semiconductor_device",
        "tool_used": "devsim",
        "domain": "Circuits",
        "system_type": result.get("system_type", "Semiconductor Device"),
        "solver_name": "devsim",
...
def run_semiconductor_device_pipeline(question, call_llm, task_id, tool="devsim", _prebuilt_input=None):
    if _prebuilt_input:
    ...
    result = {
        "system_type": plan.get("system_type", "Semiconductor Device"),
        "metrics": plan.get("metrics", []),
        "assumptions": plan.get("assumptions", []),
        "plain_summary": plan.get("plain_summary", "Device analysis completed."),
        "status": "completed",
    }
    return _to_standardized(result)


def run_semiconductor_device_pipeline_stream(question, call_llm, task_id, tool="devsim", _prebuilt_input=None):
    ...
    yield {"stage": "execution", "status": "start", "tool": "devsim"}
    result = {
        "system_type": plan.get("system_type", "Semiconductor Device"),
        "metrics": plan.get("metrics", []),
        "assumptions": plan.get("assumptions", []),
    ...
    yield {"stage": "execution", "status": "done", "tool": "devsim"}
